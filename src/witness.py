...
class WitnessState():
    ...
    def _get_segment_pattern(self):
        segments_v = []
        segments_h = []

        for i in range(self._line_tip - 1, self._line_tip + 1):
            for j in range(self._column_tip - 1, self._column_tip + 2):
                if i < 0 or i >= self._v_seg.shape[0] or j < 0 or j >= self._v_seg.shape[1]:
                    segments_v.append(-1)    
                else:
                    segments_v.append(self._v_seg[i][j])

        for i in range(self._line_tip - 1, self._line_tip + 2):
            for j in range(self._column_tip - 1, self._column_tip + 1):
                if i < 0 or i >= self._h_seg.shape[0] or j < 0 or j >= self._h_seg.shape[1]:
                    segments_h.append(-1)    
                else:
                    segments_h.append(self._h_seg[i][j])
        
        return segments_h, segments_v


    def get_pattern(self):
        segments_h, segments_v = self._get_segment_pattern()
        
        color = []
        for i in range(self._line_tip - 1, self._line_tip + 1):
            for j in range(self._column_tip - 1, self._column_tip + 1):
                if i < 0 or i >= self._lines or j < 0 or j >= self._columns:
                    color.append(-1.0)
                    continue

                if self._cells[i][j] != 0:
                    color.append(self._cells[i][j])
                else:
                    color.append(0.0)

        return tuple(color + segments_h + segments_v)
    
    def get_reversed_pattern(self):        
        segments_h, segments_v = self._get_segment_pattern()
        ...
        for i in range(self._line_tip - 1, self._line_tip + 1):
            for j in range(self._column_tip - 1, self._column_tip + 1):
                if i < 0 or i >= self._lines or j < 0 or j >= self._columns:
                    color_reversed.append(-1.0)
                    color.append(-1.0)
                    continue

                if self._cells[i][j] != 0:
                    color_reversed.append(self._cells[i][j] % 2 + 1)
                    color.append(self._cells[i][j])
                else:
                    color_reversed.append(0.0)
                    color.append(0.0)

        return tuple(color + segments_h + segments_v), tuple(color_reversed + segments_h + segments_v)
...
#             return actions
    ...
    def read_state(self, filename):
        ...
        self._dot_locations = [] # Initialize empty list
        if hasattr(self, '_cells'): # Check if _cells exists
            for r in range(self._lines):
                for c in range(self._columns):
                    cell_color = int(self._cells[r][c])
                    if cell_color > 0: # We only care about non-neutral dots
                        # Store row, column index, and color
                        self._dot_locations.append({'r': r, 'c': c, 'color': cell_color})
